...
$select_product = mysqli_query($con, "SELECT p.*, g.nombre FROM padres p
INNER JOIN generos g on g.id_g = p.genero ") or die('query failed');
while($proyecto = mysqli_fetch_assoc($select_product)){
	
      ?>

		<tr>
			<td><?php echo $proyecto['id_p']; ?></td>
			<td><?php echo $proyecto['nombre_p']; ?></td>
			<td><?php echo $proyecto['user_p']; ?></td>
...
<h2 class="text-center text-uppercase">Hijos:</h2>
...
$select_product = mysqli_query($con, "SELECT h.*, g.nombre, p.nombre_p FROM hijos h INNER JOIN padres p ON h.id_parentesco = p.id_p 
    INNER JOIN generos g on  g.id_g = h.genero ") or die('query failed');

$select = mysqli_query($con, "SELECT h.*, p.nombre_p FROM hijos h INNER JOIN padres p ON h.id_parentesco2 = p.id_p") or die('query failed');
